...
function table(json) {
  ...
  if (!qtd == 1) {
    tr = $('<tr/>');
    tr.append("<td class = 'nome'>" + json.nome + "</td>");
    tr.append("<td class = 'preco centerTd'> " + converteFloatMoeda(json.preco) + "</td>");
    ...
    tr.append("<td class = 'subtotal centerTd' id = 'subtotal'><span class = 'spanSub'>" + converteFloatMoeda(json.preco) + "</span></td>");
...
$(document).ready(function () {
  $("#tabelaitem").on('click', '.informacao', function () {
    ...
    $.ajax({
      type: "GET",
      url: "/api/venda/getDesc?desc=" + nome,
      data: $('#produto').serialize(),
      datatype: 'json',
      contentType: "application/json; charset=utf-8",
